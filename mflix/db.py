...
def update_comment(comment_id, user_email, text, date):
    ...
    response = db.comments.update_one({"_id": comment_id, "email": user_email}, {"$set": {"text": text, "date": date}})
...
def add_user(name, email, hashedpw):
    ...
    try:
        ...
        db.users.with_options(write_concern=WriteConcern(w=2)).insert_one({"name": name, "email":email, "password":hashedpw})
